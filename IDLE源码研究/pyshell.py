...
# 的有效参数...下面的认知呼叫定义如下。
# https://msdn . Microsoft . com/en-us/library/windows/desktop/dn 280512(v = vs . 85)。文件
...
if TkVersion < 8.5:
    root = Tk()  # otherwise create root in main，否则在main中创建根目录
    root.withdraw()
...
import warnings
#内部模块导入
from idlelib.colorizer import ColorDelegator#代码高亮部分
from idlelib.config import idleConf#获取用户自定义配置
from idlelib.delegator import Delegator
from idlelib import debugger
from idlelib import debugger_r#以上用于调试
...
# Default for testing; defaults to True in main() for running.，测试的默认值；在main()中默认为True以运行。
use_subprocess = False

HOST = '127.0.0.1' # python execution server on localhost loopback，本地主机回环上的python执行服务器
PORT = 0  # someday pass in host, port for remote debug capability，有朝一日，通过主机，端口进行远程调试

try:  # In case IDLE started with -n.，以防IDLE从-n开始。
    eof = 'Ctrl-D (end-of-file)'
    exit.eof = eof
    quit.eof = eof
except NameError: # In case python started with -S.，以防python以-S开头。
...
'''
#覆盖警告模块以写入warning_stream。初始化以发送空闲 #控制台的内部警告。ScriptBinding.check _语法()将 
#临时将流重定向到外壳窗口，以便在以下情况下显示警告 
#检查用户代码。
'''
...
def idle_showwarning(
    ...
    """
    '''
    显示空闲格式警告(在替换warnings.showwarning之后)。 
    不同之处在于调用的格式化程序、file=None替换，可以是无，捕获结果AttributeError，以及硬编码提示的输出。
    '''
    if file is None:
        file = warning_stream
    try:
...
def extended_linecache_checkcache(filename=None,
    ...
    """
    '''扩展linecache.checkcache以保留< pyshell#...>条目
     与其重复linecache代码，不如修补它以保存 < pyshell#...>条目，调用原来的linecache.checkcache() 
     (跳过它们)，然后恢复保存的条目。 orig_checkcache在定义时绑定到原始方法，允许对其进行修补。
    '''
